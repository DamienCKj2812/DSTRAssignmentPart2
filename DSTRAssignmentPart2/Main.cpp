...
Team createTeam(int teamID, const std::string& name, const std::string& university, RegistrationType type, int basePlayerID) {
    int randomRanking = std::rand() % 100 + 1;
    Team team(teamID, name, randomRanking, university, type);
    for (int i = 0; i < 3; ++i) {
...
void displayAndFilterGameHistory() {
    std::cout << "\n=== Game History Logs ===\n";
    gameLogger.printArrayHistory();
    gameLogger.printStackHistory();

    FilterCondition filters[2];
    filters[0].field = "university";
    filters[0].value = "APU";

    std::cout << "\n=== Filtered Results (Team Name: team10, Outcome: win) ===\n";
...
void runTournamentSystem() {
    ...
    for (int i = 0; i < readyTeams.size(); ++i) {
        teamPointers.push(&readyTeams[i]);
    }
...
int main() {
    ...
    do {
        ...
        std::cout << "2. Live Stream & Spectator Queue Management\n";
        std::cout << "3. Game Result Logging & Performance History\n";
        std::cout << "0. Exit\n";
